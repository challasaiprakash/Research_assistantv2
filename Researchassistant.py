"""
🔬 Multi-Document Research Assistant — v3 (storage-fixed)
…
Stack:
  • Streamlit   — UI
...
import logging
import tempfile
import shutil                          # ← FIX: needed for temp dir cleanup
...
# ── FIX: one dedicated temp folder for this app, isolated from the OS /tmp ──
APP_TMP_DIR = os.path.join(tempfile.gettempdir(), "research_assistant_uploads")
...
# ── FIX: purge all files written by a previous Streamlit session ─────────────
# Each Streamlit Cloud container restart leaves stale files in APP_TMP_DIR.
# We wipe them once per Python process startup (not per rerun) so the disk
# stays clean across restarts without touching files the current run needs.
def _purge_stale_tmp():
    """Delete everything in APP_TMP_DIR that is older than 2 hours."""
    import time
    cutoff = time.time() - 7200   # 2 hours
    for entry in os.scandir(APP_TMP_DIR):
        try:
...
_purge_stale_tmp()   # runs once when the Python worker starts


# ── FIX: helper — write a file to APP_TMP_DIR and return its path ────────────
def _tmp_path(filename: str) -> str:
    """Return a safe, unique path inside APP_TMP_DIR for the given filename."""
    return os.path.join(APP_TMP_DIR, filename)


# ── FIX: helper — delete a temp file silently ────────────────────────────────
...
# Extract each article's real title
...
def _pdf_title_from_doc(doc, fallback_filename: str) -> str:
    ...
    try:
        ...
        if spans:
            ...
            top = [s for s in spans if s[0] >= max_size - 0.5]
...
def extract_titles_fast(file_signatures: tuple, _uploaded_files: list) -> list:
    ...
    for uf in _uploaded_files:
        ...
        path = _tmp_path(name)                          # ← FIX: use APP_TMP_DIR
        ...
        try:
            if ext == ".pdf":
                ...
                doc.close()                             # ← FIX: explicitly close fitz doc
            elif ext in (".doc", ".docx"):
                import docx2txt
                titles.append(_guess_title_from_text(docx2txt.process(path) or "", name))
            else:
                titles.append(_guess_title_from_text(
                    uf.getvalue().decode("utf-8", errors="ignore"), name))
        except Exception:
            titles.append(Path(name).stem)
        finally:
            _remove_tmp(path)                           # ← FIX: always delete after reading
...
def process_documents(file_signatures: tuple, _uploaded_files: list):
    ...
    for uf in _uploaded_files:
        ...
        try:
            if ext == ".pdf":
                ...
                pages = [(page.get_text() or "") for page in fitz_doc]
                fitz_doc.close()                        # ← FIX: close fitz doc before removing file
            ...
            _remove_tmp(path)                           # ← FIX: always delete after indexing
    ...
    # Embed in small batches to avoid memory spike on 1 GB Streamlit Cloud tier.
...
def count_references_in_text(text: str) -> int:
    ...
    if nums:
        ints = [int(n) for n in nums if int(n) < 2000]
        if ints:
            return max(ints)
...
def retrieve_and_format(vectorstore, question: str, name_to_num: dict, name_to_title: dict, k: int = TOP_K):
    ...
    for doc_name, contents in ordered:
        ...
        title = name_to_title.get(doc_name, doc_name)
        formatted += f"\n\n{'='*60}\nSOURCE [{num}]: {title}\n{'='*60}\n"
        formatted += "\n---\n".join(contents)
        label = f"[{num}] {title}"
        if pages.get(doc_name):
            label += f"  (pp. {', '.join(str(p + 1) for p in sorted(pages[doc_name]))})"
        if doc_name in best_score:
...
<style>
    ...
    [data-testid="stChatMessage"] ol { margin: 0.2rem 0 0.4rem 1.1rem; }
...
# ── FIX: track which file set was last indexed so we can detect changes ──────
if "last_signatures" not in st.session_state:
    st.session_state.last_signatures = None
...
# ── FIX: when the file set changes, wipe ALL cached resources ────────────────
# Without this, every new upload creates a brand-new vectorstore in memory
# while old ones stay alive — on Streamlit Cloud's 1 GB limit this causes OOM.
if st.session_state.last_signatures != signatures:
    st.cache_resource.clear()               # free old vectorstores + LLM clients
    st.session_state.last_signatures = signatures
    log.info("File set changed — cache cleared. New signatures: %s", signatures)

# 1) Fast title read (first page only)
...
session_title = ("Research Assistant — "
                 + ", ".join(article_names[:3])
                 + (" …" if len(article_names) > 3 else ""))
